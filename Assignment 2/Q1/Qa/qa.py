...
def main():
    ...
    testDir=sys.argv[2]
    filePos=glob.glob(trainingDir+"/pos"+"/*.txt")
    fileNeg=glob.glob(trainingDir+"/neg"+"/*.txt")

    print("\nModel Training Started")
    phi,theta,_=train(trainingDir+"/pos",trainingDir+"/neg")

    print("Model Training Completed")
    results1=testConfuMat(trainingDir+"/pos",trainingDir+"/neg",phi,theta)
    results2=testConfuMat(testDir+"/pos",testDir+"/neg",phi,theta)

    os.chdir("..")
    file1="results1.pickle"
    fileobj1=open(file1,"wb")
    pickle.dump(results1,fileobj1)
    fileobj1.close()

    file2="results2.pickle"
    fileobj2=open(file2,"wb")
    pickle.dump(results2,fileobj2)
    fileobj2.close()

    print("\nAccurance over Training Data: ",findAcc(results1))
    print("Accurance over Test Data: ",findAcc(results2),"\n")
